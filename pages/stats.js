...
export default function Stats({ stats }) {
  ...
  return (
    <>
      <h1 className='mb-8 text-4xl font-bold text-center text-gray-100'>Stats - All Devices</h1>
      <div className='mb-6 space-y-6 lg:space-y-0 lg:grid lg:grid-cols-2 lg:gap-4'>
        <div className='flex justify-between p-4 rounded bg-aex-accent'>
          <svg fill="currentColor" viewBox="0 0 256 256" className="w-12 h-12 text-gray-900">
            ...
            <rect x="64" y="24" width="128" height="208" rx="16" stroke-width="16" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" fill="none" />
            <path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="16" d="M64 56h128M64 200h128" />
          </svg>
          <div className='flex flex-col text-right'>
            <p className='text-sm text-gray-700'>Total Devices</p>
            <p className='text-2xl font-bold text-gray-800'>{numberFormatter.format(stats.deviceCountList.length)}</p>
          </div>
        ...
        <div className='flex justify-between p-4 rounded justify-items-center bg-aex-accent'>
          <svg fill="currentColor" viewBox="0 0 256 256" className="w-12 h-12 text-gray-900 ">
            <path fill="none" d="M0 0h256v256H0z" />
            <path d="M176 128h48a8 8 0 0 1 8 8v64a8 8 0 0 1-8 8H32a8 8 0 0 1-8-8v-64a8 8 0 0 1 8-8h48M128 24v104" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="16" />
            <path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="16" d="m80 80 48 48 48-48" />
            <circle cx="188" cy="168" r="12" />
          </svg>
          <div className='flex flex-col text-right'>
            <p className='text-sm'>Total Downloads</p>
            <p className='text-2xl font-bold'>{numberFormatter.format(stats.totalInstallations)}</p>
          </div>
        </div>
      </div>
      <div className="space-y-10 lg:space-y-0 lg:grid lg:grid-cols-2 lg:gap-4">
        <div className="">
          <h1 className="mb-4 text-2xl font-medium text-center text-gray-100">Popular Devices</h1>
            ...
            <table className="min-w-full divide-y divide-aex-400 sm:rounded-md">
              ...
              <tbody className=" divide-y divide-aex-400 bg-[#332e4e]">
                {stats.deviceCountList.map((device, deviceIdx) => (
                  <tr key={device.name} >
                    <td className="px-6 py-4 text-sm font-medium whitespace-nowrap">{deviceIdx + 1}</td>
                    <td className="px-6 py-4 text-sm text-aex-accent whitespace-nowrap">
                      <Link href={`/stats/${device.name}`}>
                        <a>{`${device.model} (${device.name})`}</a>
                      </Link></td>
                    <td className="px-6 py-4 text-sm whitespace-nowrap">{numberFormatter.format(device.count)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
