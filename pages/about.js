...
const contributors = [
  ...
  {
    "name": "Sagar Makhar",
    "description": "Realme X (RMX1901) & ASUS Zenfone Max Pro M1 (X00TD) Maintainer",
    "bio": "",
  ...
  return (
    <div>
      <div className='mb-8 text-center'>
        ...
        <p className='text-gray-300'>An Android custom ROM that aims to provide useful customizations with stability.</p>
        <p className='text-gray-300'>It was stated because of why why whydsfsdaf 2017.</p>
      </div>
        ...
        {teamList.map((feature) => (
          <div className='transition-colors border-2 shadow-md md:rounded-md bg-aex-300 border-aex-300 hover:border-aex-accent ' key={feature.name} >
            ...
            <div className='flex justify-center p-4 space-x-4'>
              {
                feature.github ? <a href={feature.github} className="text-gray-100 hover:text-aex-accent">
                  <span className="sr-only">{links[0].name}</span>
                  <GitHubIcon className="w-6 h-6" aria-hidden="true" />
                </a> : null
              }
              {
                feature.xda ? <a href={feature.xda} className="text-gray-100 hover:text-aex-accent">
                  <span className="sr-only">{links[1].name}</span>
                  <XDAIcon className="w-6 h-6" aria-hidden="true" />
                </a> : null
              }
              {
                feature.telegram ? <a href={feature.telegram} className="text-gray-100 hover:text-aex-accent">
